...
from crud_3 import get_vehicles as get_vehicles_3, get_vehicle as get_vehicle_3, create_vehicle as create_vehicle_3, update_vehicle as update_vehicle_3, delete_vehicle as delete_vehicle_3
from crud_4 import create_vehicle, update_vehicle, delete_vehicle, get_vehicle, get_vehicles as get_vehicles_4
from database_2 import get_db as get_db_2
from database_1 import get_db as get_db_1
from database_3 import get_db as get_db_3
from database_4 import get_db as get_db_4
...
def index_3():
    ...
    allowed_sort_fields = ['vehicle_type', 'brand', 'commercial_type', 'group_number',
                          'serial_number', 'work_with', 'weight', 'power', 'kilometers',
                          'hours', 'engine_type', 'body_type', 'first_release_date',
                          'purchase_date', 'technical_inspection_date', 'technical_operation_date']
    
    if sort_by not in allowed_sort_fields:
        sort_by = 'brand'
    
    vehicles = get_vehicles_3(db, search=search_term, sort_by=sort_by, direction=sort_order)
...
def index_4():
    ...
    vehicles = get_vehicles_4(db, sort_by=sort_by, direction=sort_order, search_query=search_term)
...
@app.route('/add_vehicle_3', methods=['GET', 'POST'])
def add_vehicle_3():
    if request.method == 'POST':
        try:
            db = next(get_db_3())
            vehicle_data = {
                ...
                'kilometers': request.form.get('kilometers', type=int),
                'payload': float(request.form['payload']),
                'gvw': float(request.form['gvw']),
                'mam': float(request.form['mam']),
                'registration_date': datetime.strptime(request.form['registration_date'], '%Y-%m-%d').date(),
                'first_registration_date': datetime.strptime(request.form['first_registration_date'], '%Y-%m-%d').date(),
                'technical_inspection_date': datetime.strptime(request.form.get('technical_inspection_date', ''), '%Y-%m-%d').date() if request.form.get('technical_inspection_date') else None,
                'technical_operation_date': datetime.strptime(request.form.get('technical_operation_date', ''), '%Y-%m-%d').date() if request.form.get('technical_operation_date') else None,
                'comments': request.form.get('comments')
            }
            
            create_vehicle_3(db, **vehicle_data)
            flash('Véhicule ajouté avec succès!', 'success')
            return redirect(url_for('index_3'))
        except ValueError as e:
            flash(f'Erreur de format de données: {str(e)}', 'error')
            return redirect(url_for('add_vehicle_3'))
        except Exception as e:
            flash(f'Erreur lors de l\'ajout du véhicule: {str(e)}', 'error')
            return redirect(url_for('add_vehicle_3'))
    
    return render_template('add_vehicle_3.html')

@app.route('/add_vehicle_4', methods=['GET', 'POST'])
def add_vehicle_4():
    if request.method == 'POST':
        try:
            # Validate required fields
            required_fields = ['vehicle_type', 'brand', 'commercial_type', 'serial_number', 
                             'power', 'kilometers', 'hours', 'engine_type', 'body_type', 
                             'first_release_date', 'purchase_date']
            
            missing_fields = [field for field in required_fields 
                            if not request.form.get(field) and request.form.get(field) != '0']
            ...
            vehicle_data = {
                ...
                'power': float(request.form['power']) if request.form.get('power') else 0.0,
                'kilometers': int(request.form['kilometers']) if request.form.get('kilometers') else 0,
                'hours': int(request.form['hours']) if request.form.get('hours') else 0,
                'engine_type': request.form['engine_type'],
                'body_type': request.form['body_type'],
                'first_release_date': request.form['first_release_date'],
                'purchase_date': request.form['purchase_date'],
                'technical_inspection_date': request.form['technical_inspection_date'] if request.form.get('technical_inspection_date') else None,
                'technical_operation_date': request.form['technical_operation_date'] if request.form.get('technical_operation_date') else None,
                'comments': request.form.get('comments')
            }
            
            db = next(get_db_4())
            create_vehicle(db, vehicle_data)
            flash('Véhicule ajouté avec succès!', 'success')
            return redirect(url_for('index_4'))
            
        except ValueError as e:
            flash(f'Erreur de format de données: {str(e)}', 'error')
            return redirect(url_for('add_vehicle_4'))
        except Exception as e:
            flash(f'Erreur lors de l\'ajout du véhicule: {str(e)}', 'error')
            return redirect(url_for('add_vehicle_4'))
            
    return render_template('add_vehicle_4.html')

@app.route('/edit_vehicle_4/<int:id>', methods=['GET', 'POST'])
def edit_vehicle_4(id):
    db = next(get_db_4())
    ...
    if request.method == 'POST':
        try:
            ...
            vehicle_data = {
                ...
                'engine_type': request.form['engine_type'],
                'body_type': request.form['body_type'],
                'first_release_date': datetime.strptime(request.form['first_release_date'], '%Y-%m-%d').date(),
                'purchase_date': datetime.strptime(request.form['purchase_date'], '%Y-%m-%d').date(),
                'technical_inspection_date': datetime.strptime(request.form['technical_inspection_date'], '%Y-%m-%d').date() if request.form.get('technical_inspection_date') else None,
                'technical_operation_date': datetime.strptime(request.form['technical_operation_date'], '%Y-%m-%d').date() if request.form.get('technical_operation_date') else None,
                'comments': request.form.get('comments')
            }
            
            update_vehicle(db, id, vehicle_data)
            flash('Véhicule mis à jour avec succès!', 'success')
            return redirect(url_for('index_4'))
            
        except ValueError as e:
            flash(f'Erreur de format de données: {str(e)}', 'error')
            return render_template('edit_vehicle_4.html', vehicle=vehicle)
        except Exception as e:
...
@app.route('/edit_vehicle_3/<int:id>', methods=['GET', 'POST'])
def edit_vehicle_3(id):
    db = next(get_db_3())
    if request.method == 'POST':
        try:
            vehicle_data = {
            ...
            update_vehicle_3(db, id, **vehicle_data)
            flash('Véhicule modifié avec succès!', 'success')
            return redirect(url_for('index_3'))
        except ValueError as e:
            flash(f'Erreur de format de données: {str(e)}', 'error')
            return redirect(url_for('edit_vehicle_3', id=id))
        except Exception as e:
            flash(f'Erreur lors de la modification du véhicule: {str(e)}', 'error')
            return redirect(url_for('edit_vehicle_3', id=id))
    
    vehicle = get_vehicle_3(db, id)
    if vehicle is None:
        flash('Véhicule non trouvé.', 'error')
        return redirect(url_for('index_3'))
    
    return render_template('edit_vehicle_3.html', vehicle=vehicle)
...
@app.route('/delete_vehicle_3/<int:id>', methods=['GET', 'POST', 'DELETE'])
def delete_vehicle_route_3(id):
    try:
        db = next(get_db_3())
        delete_vehicle_3(db, id)
        flash('Véhicule supprimé avec succès!', 'success')
    except Exception as e:
        flash(f'Erreur lors de la suppression du véhicule: {str(e)}', 'error')
    return redirect(url_for('index_3'))

@app.route('/delete_vehicle_4/<int:id>', methods=['POST'])
def delete_vehicle_route_4(id):
    try:
        db = next(get_db_4())
        delete_vehicle(db, id)
        flash('Véhicule supprimé avec succès!', 'success')
    except Exception as e:
        flash(f'Erreur lors de la suppression du véhicule: {str(e)}', 'error')
    return redirect(url_for('index_4'))
