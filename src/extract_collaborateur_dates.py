...
class CollaborateurDateExtractor:
    ...
    def parse_date(self, date_str) -> Optional[date]:
        ...
        if (
            ...
            or str(date_str).upper()
            in ["A PASSER", "DISPENSÉ", "ABSENT", "EN COURS"]
        ):
            return None
    ...
    def extract_visite_medicale(self):
        ...
        try:
            ...
            logger.info(
                f"Extracted medical visit dates for {len(self.collaborateurs_data)} employees"
            )
            ...
            # CACES dates are typically later in the line
                            ...
                            if len(dates) >= 4:
                                ...
                                for i in range(1, min(6, len(dates))):
                                    ...
                                    if (
                                        potential_ifo and potential_ifo.year >= 2000
                                    ):  # FIMO dates should be recent
                                        ifo_date = potential_ifo
                                        break

                                # Look for CACES dates
                                for i in range(6, len(dates)):
                                    potential_caces = self.parse_date(dates[i])
                                    if (
                                        potential_caces
                                        and potential_caces.year >= 2000
                                    ):
        ...
        try:
            ...
            for line in lines:
                ...
                if any(
                    ...
                    for header in [
                        "FORMATION VALIDE",
                        "FORMATION A ECHEANCE",
                        "FORMATION A PROGRAMMER",
                        "ARRET LONGUE MALADIE",
                        "FORMATIONS",
                        "BH-HF/BO-HO",
                        "AIPR",
                        "SECOURISTES",
    ...
    def print_extracted_data(self):
        ...
        print("\nExtracted Data:")
        print(
            "Nom\tPrénom\tIFO\tCACES\tAIRR\tHGO\tBO\tVisite "
            "Médicale\tBrevet Secourisme"
        )
        print("-" * 100)

        for _, data in self.collaborateurs_data.items():
            print(
                f"{data['nom']}\t{data['prenom']}\t{data['ifo'] or ''}\t"
                f"{data['caces'] or ''}\t{data['airr'] or ''}\t"
                f"{data['hgo'] or ''}\t{data['bo'] or ''}\t"
                f"{data['visite_med'] or ''}\t{data['brevet_secour'] or ''}"
            )
