...
if not RECIPIENT_EMAIL_2:
    logger.warning(
        "RECIPIENT_EMAIL_2 not configured. Second recipient notifications will be disabled."
    )
    ...
    try:
        ...
        for key, notif in notifications_by_row.items():
            subject, body = generate_email_content(
                notif["row_data"], notif["due_items"]
            )
            ...
            if NOTIFY_DRY_RUN:
                logger.info(
                    f"[DRY RUN] Would send email to {RECIPIENT_EMAIL} for table {notif['table']} row {notif['row_id']}:\nSubject: {subject}\nBody:\n{body}"
                )
                urgent_items = [
                    item for item in notif["due_items"] if item["days_until"] <= 4
                ]
                if RECIPIENT_EMAIL_2 and urgent_items:
                    logger.info(
                        f"[DRY RUN] Would send URGENT email to {RECIPIENT_EMAIL_2} for table {notif['table']} row {notif['row_id']}"
                    )
                continue
            try:
                ...
                if (
                    ...
                    logger.error(
                        "SMTP server or port not configured. Skipping email send."
                    )
                    continue
                logger.info(
                    ...
                    with smtplib.SMTP_SSL(smtp_server, smtp_port, timeout=30) as server:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"SMTP login failed due to non-ASCII credentials: {e}"
                                )
                                continue
                        ...
                        msg = MIMEMultipart()
                        msg["From"] = formataddr(
                            (str(Header("Alerte Collaborateur", "utf-8")), safe_from)
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Notification email sent to {RECIPIENT_EMAIL} for table {notif['table']} row {notif['row_id']}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Email sendmail error: {e}\nFrom: {repr(msg['From'])}\nTo: {repr(msg['To'])}\nSubject: {repr(msg['Subject'])}\nBody: {repr(safe_body)}"
                            )
                        # Urgent notification to second recipient
                        urgent_items = [
                            item
                            for item in notif["due_items"]
                            if item["days_until"] <= 4
                        ]
                        if RECIPIENT_EMAIL_2 and urgent_items:
                            ...
                            msg2["From"] = formataddr(
                                (
                                    str(Header("Alerte Collaborateur", "utf-8")),
                                    safe_from2,
                                )
                            )
                            msg2["To"] = formataddr(
                                (str(Header("", "utf-8")), safe_to2)
                            )
                            ...
                            try:
                                try:
                                    server.send_message(
                                        msg2,
                                        from_addr=safe_from2,
                                        to_addrs=[safe_to2],
                                        mail_options=("SMTPUTF8",),
                                    )
                                except (UnicodeEncodeError, smtplib.SMTPException):
                                    server.sendmail(
                                        safe_from2, [safe_to2], msg2.as_bytes()
                                    )
                                logger.info(
                                    f"Urgent notification email sent to {RECIPIENT_EMAIL_2} for table {notif['table']} row {notif['row_id']}"
                                )
                            except Exception as e:
                                logger.error(
                                    f"Urgent email sendmail error: {e}\nFrom: {repr(msg2['From'])}\nTo: {repr(msg2['To'])}\nSubject: {repr(msg2['Subject'])}\nBody: {repr(safe_urgent_body)}"
                                )
                except (socket.gaierror, smtplib.SMTPConnectError, OSError) as e:
                    logger.error(
                        f"SMTP connection failed for host {smtp_server}:{smtp_port}: {e}. Skipping email send."
                    )
                    continue
            except Exception as e:
                logger.exception(
                    f"Error sending notification for table {notif['table']} row {notif['row_id']}"
                )
                continue
...
def send_notification_email(server, vehicle, notifications):
    ...
    try:
        ...
        msg = MIMEMultipart()
        msg["From"] = formataddr(
            (str(Header("Alerte Collaborateur", "utf-8")), str(SENDER_EMAIL or ""))
        )
        ...
        try:
            ...
            logger.info(
                f"Notification email sent to {RECIPIENT_EMAIL} for vehicle {vehicle.license_plate}"
            )
        except Exception as e:
            logger.error(
        ...
        if RECIPIENT_EMAIL_2:
            urgent_notifications = [
                notif for notif in notifications if notif["days_until"] <= 4
            ]

            if urgent_notifications:
                # Generate content for urgent notifications only
                urgent_subject, urgent_body = generate_email_content(
                    vehicle, urgent_notifications
                )
                ...
                safe_urgent_subject_v = clean_value(
                    str(f"URGENT - {urgent_subject}" or "")
                )
                safe_urgent_body_v = clean_value(str(urgent_body))
                msg2["From"] = formataddr(
                    (str(Header("Alerte Collaborateur", "utf-8")), safe_from_v2)
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Urgent notification email sent to {RECIPIENT_EMAIL_2} for vehicle {vehicle.license_plate} ({len(urgent_notifications)} urgent inspection(s))"
                    )
                except Exception as e:
                    logger.error(
        ...
        logger.error(
            f"Failed to send notification email for vehicle {vehicle.license_plate}: {e}"
        )
        raise
