...
class CollaborateurSeeder:
    ...
    def parse_date(self, date_str) -> Optional[date]:
        ...
        if not date_str or pd.isna(date_str) or str(date_str).strip() == '' or str(date_str).upper() in ['A PASSER', 'DISPENSÉ', 'ABSENT']:
            return None
    ...
    def load_apst_data(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.collaborateurs_data)} personnel records from APST")
                    ...
                    if len(parts) >= 2:
                        ...
                        if nom:
                            ...
                            if key in self.collaborateurs_data:
                                ...
                                if len(dates) > 3:  # If multiple dates, later ones might be certifications
                                    self.collaborateurs_data[key]['caces'] = self.parse_date(dates[3])
                                elif len(dates) > 1:
                                    self.collaborateurs_data[key]['caces'] = self.parse_date(dates[1])
                            else:
                                ...
                                self.collaborateurs_data[key] = {
                                    ...
                                    'bo': None,
                                    'brevet_secour': None,
                                    'commentaire': 'CACES data available' if dates else None
                                }
    ...
    def load_planning_data(self):
        ...
        try:
            ...
            for _, row in df.iterrows():
                if pd.notna(row.iloc[0]) and str(row.iloc[0]).strip() and len(str(row.iloc[0]).strip()) > 2:
                    nom_raw = str(row.iloc[0]).strip()
                    prenom_raw = str(row.iloc[1]).strip() if len(row) > 1 and pd.notna(row.iloc[1]) else ''
